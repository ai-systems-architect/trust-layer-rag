...
Averaged across all questions in the evaluation set. Measures whether the retriever
finds all relevant chunks within the top-k results — independent of generation quality.
...
Where `rank_i` is the position of the first relevant chunk for question `i`. If no
relevant chunk appears in top-k, that question contributes 0. Measures whether the
most relevant chunk surfaces near the top of results.
...
Where IDCG is the ideal DCG assuming perfect ranking. Accounts for multiple relevant
chunks at different rank positions — more nuanced than MRR for queries with several
relevant chunks.

### Faithfulness (RAGAs — primary signal)

Measures whether every claim in the generated answer is grounded in the retrieved
context. Score 0–1. A score of 1.0 means no hallucination. Primary quality signal
for this use case — a compliance assistant that fabricates guidance is worse than
one that refuses to answer.

### Context Precision (RAGAs — primary signal)

Measures whether retrieved chunks that were actually used in the answer are ranked
higher than unused chunks. Score 0–1. High precision means Cohere reranking is
surfacing the right chunks to the top.

### Context Recall (RAGAs)

Measures whether the retrieved context contains all information needed to answer the
question. Score 0–1.

### Answer Relevancy (RAGAs)

Measures whether the generated answer directly addresses the question. Score 0–1.
Below target (0.56 vs 0.70 threshold) due to system prompt compliance hedging —
documented in DL-020. Not tuned as fixing requires weakening safety behavior.

---

## Why Faithfulness and Context Precision Are the Primary Signals

For a governed compliance assistant the cost of hallucination exceeds the cost of
over-hedging. Faithfulness 0.90 confirms answers are grounded in retrieved context.
Context precision 0.94 confirms Cohere reranking surfaces relevant chunks. Answer
relevancy is deliberately deprioritized — a compliance system that qualifies its
answers is behaving correctly.

...
## DL-029 — Env-gated debug instrumentation for worked example regeneration
**Date:** 2026-04-28
**Status:** Accepted

**Decision:** Worked example tables in README.md are regenerated by running
`scripts/run_worked_examples.py` with `DEBUG_PIPELINE=true` set in the
environment. Debug instrumentation lives permanently in `pipeline.py`
(gated by the env var, zero cost when unset) and dispatches to formatting
helpers in `evaluation/worked_examples_debug.py`.

**Rationale:**
1. Worked examples are not one-shot artifacts — they need refresh whenever
   pipeline behaviour changes (corpus updates, retrieval tuning, prompt
   revisions, model swap). Without committed tooling, each refresh
   requires copy-pasting ad-hoc instrumentation into `pipeline.py` and
   reverting after — friction that delays refreshes and introduces drift
   between README and current behaviour.
2. Production hot-path concerns are addressed by the env var gate. The
   `_DEBUG` check is a single boolean comparison evaluated at module
   load. When `DEBUG_PIPELINE` is unset, no helper is imported and no
   debug code runs.
3. Co-locating helpers in `evaluation/` keeps `pipeline.py` minimal
   (≈12 added lines total — four hook points of three lines each) and
   groups debug tooling with the existing evaluation scripts
   (`ragas_eval.py`, `retrieval_diagnostics.py`, `guardrail_test.py`).

**Alternatives considered:**
- Keep debug blocks in `scratch/` (gitignored, the original approach):
  rejected — files are local-only, lost on machine moves, friction to
  re-instrument every refresh.
- Bake full debug blocks into `pipeline.py` without a helper module:
  rejected — ≈50 lines of conditional code in the hot-path file.
- Standalone capture script that re-implements retrieval/rerank outside
  `pipeline.py`: rejected — would duplicate pipeline logic and drift
  from production behaviour over time.

**Consequences:**
- `pipeline.py` imports `os` and `time` at module load (negligible) and
  has four `if _DEBUG:` blocks of three lines each.
- Debug output format is now versioned. Changes to README table format
  must be reflected in `evaluation/worked_examples_debug.py`.
- Re-running `scripts/run_worked_examples.py` costs 6 × (embedding +
  Cohere rerank + Bedrock generate) per refresh — non-trivial but rare
